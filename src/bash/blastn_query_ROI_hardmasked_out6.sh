...
# makeblastdb, and a specific query file containing cDNA sequences.
#
...
#
# Example:
#   blastn_run "/path/to/blastdb" "/path/to/cDNA_queries" "/path/to/blastn_results" "/path/to/single_contig_list.txt" "NKG2" 4
#
# Notes: 
...
blastn_run(){
    ...
    while read genome; do
        ...
        local cmd="blastn -db \"$makeblastdb_dir/${genome}\" -query \"$query_dir\" -outfmt \"6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore sstrand\""
        ...
        # Run blastn
        eval $cmd > "$output_dir/${genome}_ROI_${gene_to_annotate}_cDNA_blastn_out6"
