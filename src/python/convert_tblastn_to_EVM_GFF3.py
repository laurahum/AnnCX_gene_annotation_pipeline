...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep 10 20:40:14 2024

@author: lahumada
"""


# This script converts formatted filtered GFF output from TBLASTN (protein) to a format compatible with EVM
#
# 1. Convert features
# The output from TBLASTN contains for features:
#     BLASTCDS
# from those features EVM (protein alignment) accepts:
#     nucleotide_to_protein_match
# all the BLASTCDS features in TBLASTN formatted filtered GFF file must be converted to nucleotide_to_protein_match features
#
# 2. Modify attributes column
# The attributes column of the TBLASTN formatted filtered GFF files must be modified according to the example provided by EVM
# Give the entries associated to the same query output alignment, in order of entry, a counter in the attributes section:
# ex. ID=match.tblastn.1 for the BLASTCDS that are associated to the first query match entry.
...
#
# Input: TBLASTN formatted filtered GFF output (with features: BLASTCDS)
# Output: TBLASTN formatted filtered EVM-compatible GFF3 file (with features: nucleotide_to_protein_match)

# Main function: convert_tblastn_to_EVM_all_files(input_dir, output_dir, source)
...
def convert_tblastn_to_EVM(input_file, output_file, source):
    """
    Convert TBLASTN formatted-filtered-GFF output to GFF3 format compatible with EVM.
    BLASTCDS -> nucleotide_to_protein_match
        - input_file (str): Path to the input TBLASTN output file
        ...
        - source (str): Name of the tool used to generate the annotation output. Possible values:
            - tblastn: run TBLASTN with protein as query
    """
            ...
            if line.strip() and not line.startswith('#'):
                ...
                if fields[2].upper() == 'BLASTCDS':
                    ...
                    if id_match and pident_match and length_match and bitscore_match:
                        ...
                        new_line = f"{seqid}\t{source}\tnucleotide_to_protein_match\t{start}\t{end}\t{bitscore}\t{strand}\t{phase}\t{new_attributes}\n"
                        outfile.write(new_line)

# Main
def convert_tblastn_to_EVM_all_files(input_dir, output_dir, source):
    '''
    Loop the convert_tblastn_to_EVM function over all the formatted filtered TBLASTN result annotation files for each genome
        - input_dir: Path to formatted-filtered-GFF TBLASTN result annotation files
        - output_dir: Path to formatted-filtered-EVM-GFF TBLASTN result annotation files
