...
# -*- coding: utf-8 -*-
"""
Created on Sun Feb 22 00:21:17 2026

@author: lahumada
"""

"""
This script will filter Minimap2 gene model results.
Since the new Minimap2 GFF3 formatter produces gene model annotations (gene → mRNA → exon), 
each gene model (labeled as 'gene' in the GFF3 files) is considered for filtering and referred 
to as 'genes' (which includes its associated features: mRNA, exons) throughout the script.

Minimap2 aligns several gene model annotation entries on the same region, with overlapping 
...
query_cov   = match_bases / (1+query_len) # Query coverage  
gaps_penalty= (align_len - match_bases) / 100 # Normalized gaps/indels

Logic: If gene models overlap, keep highest compound score. Ties broken randomly.

Main function: filter_minimap2_one_map_per_region(input_dir, output_dir)
"""
...
def calculate_minimap2_compound_score(query_len, match_bases, align_len, mapq, ms):
...
def filter_genes_same_map_region(Filepath_input, Filepath_output):
    '''Filters Minimap2 gene models by compound score when ranges overlap.
    1. Load and format GFF3
    2. Extract gene model data and calculate compound scores
    3. Split by gene model (gene+mRNA+exons)
    ...
    # Get mRNA data for compound score components
    ...
    for i in range(len(gene_rows)):
        ...
        # Find corresponding mRNA row
        mrna_attributes = None
        if i < len(mRNA_rows):
            mrna_attributes = str(mRNA_rows.iloc[i,8])
        
        # Extract compound score components from mRNA attributes
        query_len_match = re.search(r'qlen=([^;]+)', mrna_attributes)
        query_len = int(query_len_match.group(1))
        
        match_bases_match = re.search(r'nmatch=([^;]+)', mrna_attributes)
        match_bases = int(match_bases_match.group(1))
        
        align_len_match = re.search(r'alen=([^;]+)', mrna_attributes)
        align_len = int(align_len_match.group(1))
        
        mapq_match = re.search(r'mapq=([^;]+)', mrna_attributes)
        mapq = int(mapq_match.group(1))
        
        ms_match = re.search(r'ms=([^;]+)', mrna_attributes)
        ms = int(ms_match.group(1))
        
        # Calculate compound score
        compound_score = calculate_minimap2_compound_score(
...
def filter_minimap2_one_map_per_region(input_dir, output_dir):
    '''Loop over raw Minimap2 GFF3 files and apply compound score filtering'''
